...
public:
  ...
  // int start_pos[AXIS_SIZE]; // unit: 0.1 step
  // int end_pos[AXIS_SIZE];   // unit: 0.1 step
...
public:
  ...
  void addSyncMove(abce_long_t sync_pos);
  void setMove(uint8_t m_idx, float sv, float ev, float acc, float dist, xyze_float_t& axis_r, uint32_t t);
  Move *addMove(float sv, float ev, float acc, float dist, xyze_float_t& axis_r, uint32_t t);
...
public:
  ...
  // int last_mq_pos[AXIS_SIZE];
  float last_mq_pos[AXIS_SIZE];
